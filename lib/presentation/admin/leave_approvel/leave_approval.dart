...
class _LeaveApprovelState extends State<LeaveApprovel> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Fetching the leave data from Firestore (subcollection under employees)
    _leaveStream = FirebaseFirestore.instance
        .collection('employees') // Main collection
        .doc('employeeId') // Replace with actual employee ID or use an ID from your user session
        .collection('leaves') // The subcollection that holds leave data
        .snapshots(); // Stream to listen for real-time updates
  }
      ...
      child: Scaffold(
        ...
        body: StreamBuilder<QuerySnapshot>(
          ...
          builder: (context, snapshot) {
            ...
            var leaveDocuments = snapshot.data!.docs;

            return ListView.builder(
              itemCount: leaveDocuments.length,
              itemBuilder: (BuildContext context, int index) {
                Animation<double> animation = CurvedAnimation(
                  parent: _controller,
                  curve: Interval(
                    (1 / leaveDocuments.length) * index, // Animate each item sequentially
                    1.0,
                ...
                var leaveData = leaveDocuments[index].data() as Map<String, dynamic>;
                String leaveDate = leaveData['leave_date'] ?? 'N/A';
                String leaveTime = leaveData['leave_time'] ?? 'N/A';
                String leaveType = leaveData['leave_type'] ?? 'N/A';
                String leaveStatus = leaveData['leave_status'] ?? 'N/A';
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    ..._buildInfoText(screenWidth, leaveStatus),
                                    ..._buildInfoText(screenWidth, leaveType),
                                    SizedBox(height: 20),
